...
<h1>黑体辐射源智能校准系统方案配置系统</h1>
    ...
    <a-form :layout="formLayout" :model="msg" >
        ...
        >
        <a-input placeholder="请输入方案名称" disabled v-model='msg.value' />
        </a-form-item>
        <a-form-item
        label="测试数量"
        :label-col="formItemLayout.labelCol"
        ...
        >
        <a-input placeholder="请输入测试点数" disabled v-model='msg.testcount' />
        </a-form-item>
        <a-form-item
        label="时间间隔"
        :label-col="formItemLayout.labelCol"
        ...
        >
        <a-input placeholder="请输入测试时间间隔" disabled v-model='msg.testinterval' />
        </a-form-item>
        ...
        :label-col="formItemLayout.labelCol"
        :wrapper-col="formItemLayout.wrapperCol"
         v-for="(domain, index) in msg.list"
        :key="domain.key"
        :label="'温度' + Number(index+1)"
        :prop="'domains.' + index + '.tp'"
        >
        <a-input placeholder=""  disabled v-model="domain.pointtemperature" @blur="paixu"/>
        ...
        >
        <a-input placeholder="" disabled v-model='msg.flow' />
        </a-form-item>
        <a-form-item
        label="样本标准差"
        :label-col="formItemLayout.labelCol"
        ...
        >
        <a-input placeholder="" disabled v-model='msg.differentValue' />
        </a-form-item>
        <a-form-item
        label="偏差绝对值"
        :label-col="formItemLayout.labelCol"
...
mounted(){
      console.log(JSON.parse(this.$router.apps[0]._route.query.value))
      this.msg=JSON.parse(this.$router.apps[0]._route.query.value)
  }
  ,
  computed: {
    formItemLayout() {
